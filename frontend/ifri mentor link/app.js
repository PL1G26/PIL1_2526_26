...
function acceptMatch(id) {
  ...
  if(!alreadyExists) {
    ...
    state.conversations.unshift({
      ...
      color: match.color,
      preview: `Bonjour ! Notre match a été accepté 🎉 Je suis prêt·e à commencer les sessions de ${match.skill}.`,
      time: time,
      unread: 1,
      messages: [
...
function publishTestimonial() {
  ...
  const alert = document.getElementById('testi-alert');
  if(!name || !text) {
    alert.className = 'alert alert-error';
    alert.textContent = '⚠️ Remplis ton prénom et ton témoignage.';
  ...
  alert.textContent = '✅ Témoignage publié ! Merci pour ton retour.';
  alert.classList.remove('hidden');
  setTimeout(()=>alert.classList.add('hidden'),3000);
  renderTestimonials();
}
...
function renderTestimonials() {
  ...
  document.getElementById('testimonials-list').innerHTML = all.map(t=>`
...
renderTestimonials();
document.querySelectorAll('#star-rating .star').forEach(s=>{ s.style.color='#d1d5db'; s.style.cursor='pointer'; s.style.fontSize='22px'; s.style.transition='color 0.15s'; s.onmouseover=()=>{ document.querySelectorAll('#star-rating .star').forEach((ss,ii)=>{ ss.style.color = ii<=parseInt(s.dataset.val)-1 ? '#fbbf24':'#d1d5db'; }); }; s.onmouseleave=()=>{ document.querySelectorAll('#star-rating .star').forEach((ss,ii)=>{ ss.style.color = ii<currentRating ? '#f59e0b':'#d1d5db'; }); }; });
