...
async def get_all_posts(
    ...
    Filtres optionnels :
    - skill_id : filtrer par compétence
    - post_type : filtrer par type ('offer' ou 'request')
    - mode : filtrer par mode ('online', 'offline', 'both')
    """
    # Construction de la requête de base (en string brut pour pouvoir concaténer les filtres)
    query_str = """
        SELECT 
            mp.id, mp.user_id, mp.type, mp.skill_id, mp.mode,
            mp.description, mp.is_active, mp.created_at, mp.updated_at,
    ...
    for row in result:
        ...
        posts.append(_build_post_response(db, post_data))
